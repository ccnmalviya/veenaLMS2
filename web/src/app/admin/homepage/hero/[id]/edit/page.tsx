...
export default function EditHeroBannerPage() {
  ...
  const loadHeroBanner = async () => {
    try {
      setLoading(true);
      const docRef = doc(db, "hero_banners", heroId);
      const docSnap = await getDoc(docRef);
      
      if (!docSnap.exists()) {
        alert("Hero banner not found");
      ...
      setFormData({
        ...
        hideImageOnMobile: data.hideImageOnMobile ?? false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "hero_banners", heroId), heroBannerData);
      router.push("/admin/homepage/hero");
    } catch (error) {
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 max-w-4xl">
          {/* Same form structure as new page - using the same sections */}
          {/* 1. Basic Control */}
                  ...
                  <div className="space-y-4">
                    <div className="relative">
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 bg-red-600 text-white px-3 py-1 rounded-lg hover:bg-red-700 text-sm font-medium shadow-lg"
                      >
                        Delete Image
                      </button>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 mb-2">Upload a new image to replace the current one:</p>
                      <FileUpload
                        accept="image/*"
                        folder="hero-banners"
